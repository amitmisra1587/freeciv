...
static void sg_load_savefile(struct loaddata *loading)
{
  ...
  {
    const char *modname_old[] = { "Disband", "Sci_Specialists", "Tax_Specialists" };
    const char **modname;
    int j;
    bool compat;

    if (loading->coptions.size > 0) {
      modname = secfile_lookup_str_vec(loading->file, &loading->coptions.size,
                                       "savefile.city_options_vector");
      compat = FALSE;
    } else {
      modname = modname_old;
      loading->coptions.size = 3;
      compat = TRUE;
    }
                                ...
                                struct player *plr)
{
  ...
  /* Load nation style. */
  {
    ...
    if (str == NULL) {
      str = secfile_lookup_str(loading->file, "player%d.city_style_by_name",
                               plrno);
    }
                                  ...
                                  struct player *plr)
{
  ...
  if (!player_has_flag(plr, PLRF_FIRST_CITY) && ncities > 0) {
    /* Probably barbarians in an old savegame; fix up */
    BV_SET(plr->flags, PLRF_FIRST_CITY);
  }
