...
gate_dir=runs/organic_history_phase30_successor_inheritance_gate
...
python3 tools/organic_history/run_ai_game.py \
  ...
  --clean-output-dir \
  --timeout 360 \
  --extra-command 'lua cmd organic_history_gate_extra_city_done = false; function organic_history_gate_extra_city(turn, year) if not organic_history_gate_extra_city_done then organic_history_gate_extra_city_done = true; local p = find.player("Gilgamesh"); local specs = {{50, 43, "Akkad"}, {53, 43, "Lagash"}, {54, 41, "Nippur"}, {49, 42, "Eridu"}}; local c = 0; if p ~= nil then for _, spec in ipairs(specs) do local t = find.tile(spec[1], spec[2]); if t ~= nil and t:city() == nil and edit.city_create(p, t, spec[3], nil) then c = c + 1 end end end; log.normal("organic_history_gate_setup successor_extra_cities=%d", c) end; return false end; signal.connect("turn_begin", "organic_history_gate_extra_city")' \
  ...
  --extra-command 'lua cmd organic_history_dynastic_transfer_max_cities = 3' \
...
transfers = [
    int(match.group(1))
    for match in re.finditer(r'action="inherited".*transferred=(\d+)', stdout)
]
checks = {
    ...
    "cluster_transfer": max(transfers or [0]) >= 2,
    "bounded_reason": int(reasons.get("bounded_cluster_inheritance") or 0) == 1,
    "transfer_cap_logged": int(fields.get("transfer_cap", {}).get("count") or 0) > 0,
    "candidate_count_logged": int(fields.get("transfer_city_count", {}).get("count") or 0) > 0,
    "structured": summary.get("logCounts", {}).get("dynasticTransfer") == metadata.get("organicDynasticTransferLogCount"),
}
if not all(checks.values()):
    raise SystemExit(f"FAIL: Phase 30 successor inheritance checks failed: {checks}")
PY

echo "SUCCESS: Phase 30 successor inheritance gate passed"
