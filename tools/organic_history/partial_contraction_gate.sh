...
gate_dir=runs/organic_history_partial_contraction_gate
...
python3 tools/organic_history/run_ai_game.py \
  ...
  --extra-command 'lua cmd organic_history_partial_contraction_probe_enabled = true' \
...
if grep -q 'organic_history_event type=city_transferred' "$gate_dir"/probe/server_*.log; then
  echo "FAIL: partial contraction probe changed city ownership"
  exit 1
fi

if grep -q 'organic_history_secession type=secession_triggered' "$gate_dir"/probe/server_*.log; then
  echo "FAIL: partial contraction probe triggered secession"
  exit 1
fi
...
checks = {
    ...
    "logs": int(metadata.get("organicPartialContractionLogCount") or 0) > 0,
...
if not all(checks.values()):
    raise SystemExit(f"FAIL: partial contraction checks failed: {checks}")
PY

echo "SUCCESS: partial contraction gate passed"
