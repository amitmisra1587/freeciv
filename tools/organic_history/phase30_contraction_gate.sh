...
gate_dir=runs/organic_history_phase30_contraction_gate
...
python3 tools/organic_history/run_ai_game.py \
  ...
  --extra-command 'lua cmd organic_history_gate_cluster_done = false; function organic_history_gate_cluster(turn, year) if not organic_history_gate_cluster_done then organic_history_gate_cluster_done = true; local p = find.player("Narmer"); local specs = {{50, 43, "Ineb-Hedj East"}, {53, 43, "Ineb-Hedj North"}, {54, 41, "Ineb-Hedj Canal"}, {49, 42, "Ineb-Hedj South"}, {51, 41, "Ineb-Hedj West"}}; local c = 0; if p ~= nil then for _, spec in ipairs(specs) do local t = find.tile(spec[1], spec[2]); if t ~= nil and t:city() == nil and edit.city_create(p, t, spec[3], nil) then c = c + 1 end end end; log.normal("organic_history_gate_setup contraction_cluster_cities=%d", c) end; return false end; signal.connect("turn_begin", "organic_history_gate_cluster")' \
  >/dev/null
...
transfers = [
    int(match.group(1))
    for match in re.finditer(r'action="released".*transferred=(\d+)', stdout)
]
checks = {
    "success": metadata.get("success"),
    "assertions": int(metadata.get("freecivAssertionLogCount") or 0) == 0,
    "released_once": int(actions.get("released") or 0) == 1,
    "cluster_release": max(transfers or [0]) >= 2,
    "bounded_reason": int(reasons.get("bounded_cluster_release") or 0) == 1,
    "transfer_limit_logged": int(fields.get("transfer_limit", {}).get("count") or 0) > 0,
    "structured": summary.get("logCounts", {}).get("partialContraction") == metadata.get("organicPartialContractionLogCount"),
}
if not all(checks.values()):
    raise SystemExit(f"FAIL: Phase 30 contraction checks failed: {checks}")
PY

echo "SUCCESS: Phase 30 contraction gate passed"
